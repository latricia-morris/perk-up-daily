...
/**
 * SmileExercise
 * Rhythm: 4s inhale → 7s hold → 8s exhale (4-7-8 breathing)
 * Emotional tone: comforting, gentle, emotionally warm
...
const PHASES = [
  { name: "inhale", label: "Inhale", duration: 4000, scaleFrom: 0.4, scaleTo: 1.2 },
  { name: "hold", label: "Hold", duration: 7000, scaleFrom: 1.2, scaleTo: 1.2 },
...
const GRADIENT_STOPS = [
  ...
  { at: 0.5, rgb: [249, 88, 38] },   // ember (middle)
  { at: 1.0, rgb: [255, 173, 9] },   // amber (top)
];
...
const BENEFITS = {
  title: 'Benefits of "Smile"',
  subtitle: "The 4 · 7 · 8 comforting breath",
  bullets: [
    "Quiets racing thoughts and softens the emotional charge",
    "Slows the heart rate quickly, helping the body feel safe",
    "Eases the transition into sleep or deep rest",
    "Balances the autonomic nervous system in under a minute",
    "Releases tension held in the chest, throat, and jaw",
    "Gently lifts the mood by increasing oxygen circulation",
  ],
};
...
export default function SmileExercise() {
  ...
  useEffect(() => {
    ...
    const draw = (now) => {
      ...
      for (const p of particles) {
        const disp =
          1 +
          Math.sin(p.x * 2.4 + t * 0.36) * 0.08 +
          Math.cos(p.y * 2.2 + t * 0.30) * 0.07 +
          Math.sin(p.z * 2.6 + t * 0.42) * 0.06;
        const x0 = p.x * disp;
        const y0 = p.y * disp;
        const z0 = p.z * disp;
      ...
      for (const it of proj) {
        ...
        const v = p.gradPos + Math.sin(t * 0.12 + p.gradPos * 2.3) * 0.06;
  ...
  return (
    ...
    >
      <style>{`
        @keyframes smile-drift-1 { 0%,100% { transform: translate3d(-6%, -8%, 0) scale(1);} 50% { transform: translate3d(8%, 6%, 0) scale(1.1);} }
        @keyframes smile-drift-2 { 0%,100% { transform: translate3d(12%, 10%, 0) scale(1.06);} 50% { transform: translate3d(-8%, -12%, 0) scale(0.94);} }
        @keyframes smile-drift-3 { 0%,100% { transform: translate3d(6%, -16%, 0) scale(0.98);} 50% { transform: translate3d(-12%, 8%, 0) scale(1.12);} }
        @keyframes smile-blob-morph { 0%,100% { border-radius: 50%; } 20% { border-radius: 58% 42% 60% 40% / 45% 55% 45% 55%; } 40% { border-radius: 45% 55% 40% 60% / 55% 45% 55% 45%; } 60% { border-radius: 62% 38% 52% 48% / 50% 58% 42% 50%; } 80% { border-radius: 48% 52% 45% 55% / 58% 42% 55% 45%; } }
        @keyframes smile-blob-morph-2 { 0%,100% { border-radius: 58% 42% 55% 45% / 48% 52% 48% 52%; } 33% { border-radius: 42% 58% 48% 52% / 55% 45% 60% 40%; } 66% { border-radius: 55% 45% 60% 40% / 42% 58% 45% 55%; } }
        @keyframes smile-fade-in { from { opacity: 0; transform: translateY(6px);} to { opacity: 1; transform: none;} }
      `}</style>
      ...
      <div className="pointer-events-none absolute inset-0">
        <div
          ...
          style={{
            background: `radial-gradient(58% 55% at 22% 24%, ${PALETTE.amber}99, transparent 60%)`,
            filter: "blur(45px)",
            animation: "smile-drift-1 24s ease-in-out infinite",
          }}
        />
        <div
          className="absolute -inset-[20%]"
          style={{
            background: `radial-gradient(55% 55% at 78% 32%, ${PALETTE.ember}80, transparent 62%)`,
            filter: "blur(55px)",
            animation: "smile-drift-2 30s ease-in-out infinite",
          }}
        />
        <div
          className="absolute -inset-[20%]"
          style={{
            background: `radial-gradient(65% 60% at 55% 88%, ${PALETTE.rose}66, transparent 65%)`,
            filter: "blur(65px)",
            animation: "smile-drift-3 34s ease-in-out infinite",
          }}
        />
        <div
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-5 py-10 sm:py-14">
        <div
          ...
          style={{
            background:
              "linear-gradient(180deg, rgba(255,252,242,0.74) 0%, rgba(255,252,242,0.5) 100%)",
            ...
            animation: "smile-fade-in 0.7s ease-out both",
          }}
          data-testid="smile-card"
        >
          ...
          <p className="mb-6 text-sm sm:text-base" style={{ color: `${PALETTE.ink}A6` }}>
            A comforting breath that softens the whole body.
          </p>
          ...
          >
            ...
            <div
              ...
              style={{
                width: 360,
                height: 360,
                background: `radial-gradient(circle, ${PALETTE.amber}66 0%, ${PALETTE.rose}22 42%, transparent 72%)`,
                filter: "blur(40px)",
            ...
            <div
              ...
              style={{
                width: 290,
                height: 290,
                background: `radial-gradient(circle, ${PALETTE.ember}33 0%, ${PALETTE.rose}22 45%, transparent 70%)`,
                filter: "blur(24px)",
      ...
      {modalOpen && (
        <div
          data-testid="smile-benefits-modal"
        ...
        >
          <div
            ...
            style={{
              ...
              animation: "smile-fade-in 0.35s ease-out both",
