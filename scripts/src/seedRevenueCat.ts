...
const PROJECT_NAME = "Perk Up Daily";

const PRODUCT_IDENTIFIER = "perkup_premium_monthly";
const PLAY_STORE_PRODUCT_IDENTIFIER = "perkup_premium_monthly:monthly";

const PRODUCT_DISPLAY_NAME = "Perk Up Premium Monthly";
const PRODUCT_USER_FACING_TITLE = "Perk Up Premium";
const PRODUCT_DURATION = "P1M";

const APP_STORE_APP_NAME = "Perk Up Daily iOS";
const APP_STORE_BUNDLE_ID = "com.perkup.daily";
const PLAY_STORE_APP_NAME = "Perk Up Daily Android";
const PLAY_STORE_PACKAGE_NAME = "com.perkup.daily";
...
// $9.99/month — competitive for wellness apps
const PRODUCT_PRICES = [
  { amount_micros: 9990000, currency: "USD" },
  { amount_micros: 8990000, currency: "EUR" },
  { amount_micros: 7990000, currency: "GBP" },
];
...
async function seedRevenueCat() {
  ...
  const testProduct = await ensureProduct(testStoreApp, "Test Store", PRODUCT_IDENTIFIER, true);
  const iosProduct = await ensureProduct(appStoreApp, "App Store", PRODUCT_IDENTIFIER, false);
  const androidProduct = await ensureProduct(playStoreApp, "Play Store", PLAY_STORE_PRODUCT_IDENTIFIER, false);
  ...
  if (priceError) {
    if (typeof priceError === "object" && "type" in priceError && (priceError as any).type === "resource_already_exists") {
      ...
      console.warn("Could not set test store prices:", priceError);
    }
  } else {
